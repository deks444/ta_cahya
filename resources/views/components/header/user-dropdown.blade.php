<div class="relative" x-data="{
    ...
    >
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11">
            ...
            @else
                <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::check() ? Auth::user()->name : 'User') }}&background=random" alt="User" />
            @endif
        </span>

       <span class="block mr-1 font-medium text-theme-sm">{{ Auth::check() ? Auth::user()->name : 'Guest' }}</span>
        ...
        <ul class="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 dark:border-gray-800">
            @auth
                @php
                    ...
                    } elseif ($user->role === 'atlit') {
                        $profileRoute = route('atlit.profile');
                    }
                @endphp
